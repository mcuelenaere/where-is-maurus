...
func (s *Store) UpdateHeading(carID int64, ts int64, heading float64) []byte {
    ...
    ce := s.ensure(carID)
    ce.state.TSMS = ts
    if ce.state.Location == nil { ce.state.Location = &Location{} }
    ce.state.Location.Heading = heading
    ce.history.Heading = append(ce.history.Heading, TimestampedFloat{TS: ts, V: heading})
    cutoff := ts - ceWindowMs(s.window)
    s.prune(&ce.history, cutoff)
...
func (s *Store) UpdateBatteryLevel(carID int64, ts int64, soc float64) []byte {
    ...
    ce := s.ensure(carID)
    ce.state.TSMS = ts
    if ce.state.Battery == nil { ce.state.Battery = &Battery{} }
...
func (s *Store) UpdateInsideTemp(carID int64, ts int64, c float64) []byte {
    ...
    ce := s.ensure(carID)
    ce.state.TSMS = ts
    if ce.state.Climate == nil { ce.state.Climate = &Climate{} }
...
func (s *Store) UpdateTPMS(carID int64, ts int64, pos string, v float64) []byte {
    ...
    ce := s.ensure(carID)
    ce.state.TSMS = ts
    if ce.state.TPMS == nil { ce.state.TPMS = &TPMSBar{} }
    var key string
    switch pos {
    case "fl": ce.state.TPMS.FL = v; ce.history.TPMSFL = append(ce.history.TPMSFL, TimestampedFloat{TS: ts, V: v}); key = "fl"
    case "fr": ce.state.TPMS.FR = v; ce.history.TPMSFR = append(ce.history.TPMSFR, TimestampedFloat{TS: ts, V: v}); key = "fr"
    case "rl": ce.state.TPMS.RL = v; ce.history.TPMSRL = append(ce.history.TPMSRL, TimestampedFloat{TS: ts, V: v}); key = "rl"
    case "rr": ce.state.TPMS.RR = v; ce.history.TPMSRR = append(ce.history.TPMSRR, TimestampedFloat{TS: ts, V: v}); key = "rr"
    }
    cutoff := ts - ceWindowMs(s.window)
    s.prune(&ce.history, cutoff)
...
func (s *Store) UpdateRoute(carID int64, ts int64, dest *Dest, etaMin, distKM float64) []byte {
    ...
    ce := s.ensure(carID)
    ce.state.TSMS = ts
    if ce.state.Route == nil { ce.state.Route = &Route{} }
    ce.state.Route.Dest = dest
    if etaMin > 0 { ce.state.Route.ETAMin = etaMin }
    if distKM > 0 { ce.state.Route.DistKM = distKM }
